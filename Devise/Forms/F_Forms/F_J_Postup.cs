    ...
    public partial class F_J_Postup : Form
    {
            ...
            try
            {
                if (dataGridView1.RowCount > 1)
                {
                    if (dataGridView1.RowCount != dataGridView1.CurrentRow.Index)
                        textBox1.Text = new Summ_naklad().Summ("Sostav_Postupl_Naklad", "ID_Naklad_Postupl", (int)dataGridView1.CurrentRow.Cells[1].Value).ToString("0.00") + " руб.";
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (dataGridView1.CurrentRow.Index != dataGridView1.RowCount)
            {
                try
                {
                    ...
                    f.ID = int.Parse(dataGridView1.CurrentRow.Cells[1].Value.ToString());
                    f.ShowDialog();
                }
                catch { }
            }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                {
                    if (dataGridView1.RowCount != dataGridView1.CurrentRow.Index)
                        textBox1.Text = new Summ_naklad().Summ("Sostav_Postupl_Naklad", "ID_Naklad_Postupl", (int)dataGridView1.CurrentRow.Cells[1].Value).ToString();
